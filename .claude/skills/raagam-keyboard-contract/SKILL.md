...
| `data-focus-land` | **Where a surface wants the cursor to LAND first**, when that is not its first field. Read by `focusFirstField` ALONE — the section landing in `MasterFullScreen`, a Sheet opening, a `goToSection` on the current section — which takes the first marked control that is on the Tab path (a read-only box or one inside `data-focus-optional` is passed over). Tab, Enter and the arrows never read it, so it cannot reorder fields once the operator is inside. Absent, the landing is unchanged. First use (client 2026-09-21): Budget cost grids stamp every row's Rate, so Purchase Rates opens — and Yarn → Fabric switches — with the cursor on the first row's Rate instead of the first typeable cell (Currency), while FOC / Import / an INR Currency sit under `data-focus-optional`. |
