...
| Enter | the **next field**; off the last one, **save the record** (blocked while that field is invalid); on a **tick box / radio**, toggles it instead | **pick the highlight, close, stay on the field** | next row; on the last row, add a row |
...
| `data-focus-optional` | An **opt-in** control, off the default typing path: Tab and Enter step over it, ↑↓←→ and the mouse still land on it, and Space/Enter on it then works normally. For the escape-hatch toggle an operator should reach for deliberately rather than trip over. Read `[data-focus-optional]` by `cycleTab` / `enterAdvances` / `focusFirstField`, deliberately ignored by `arrowNavigate` — so it needs a focus-trapped surface (`Sheet` / `MasterFullScreen`) to work, and a spatial neighbour to be reachable from. Prefer to drop it once the operator has opted IN. First and reference use: Material ▸ Fabric ▸ Direct Purchase. |
