...
/**
 * THE ENTRY IS THE TABLE'S FIRST ROW (client 2026-09-22, screenshot 123236:
 * "cancellation la antha card la irukka data lam antha table la varanum, card
 * la vara koodathu" — the fields in that card go in the table, not in a card).
 *
 * Third home for the same six fields in three days, and each move was the
 * client's. They began as a full-width card between the header and the table
 * ("the form layout inside the entry listings show, please fix it", 2026-09-20)
 * and went into a `Sheet`; the Sheet is what was called "the card" today. What
 * the client is describing is the legacy RP screen, where a cancellation is a
 * line typed straight into the register — so the register's first row IS the
 * entry: the same columns, with a box in each cell instead of a value, and
 * "Cancel order" where the row's actions sit. No "+ New cancellation" button
 * and nothing that opens: the row is always there, which is also the
 * "editable sub-tables open with a row" rule (AGENTS.md) applied to a
 * listing that takes entries.
 *
 * ONE ROW, NOT A GRID. A cancellation is written one at a time — `cancelOrder`
 * flips the order to Cancelled and there is nothing to batch — so this is a
 ...
 *   the six boxes: Cancel No num 72 · RE No code 144 · Customer code 144
 ...
 * hover — a picker, and a phrase. Cancel No holds "(auto)" and nothing else,
 ...
 * field the row already carries, so none costs a query. A cancellation has no
 * status of its own (every row IS a cancellation) and no delivery date, so the
 * two questions are "when" and "whose".
 */
const CANCELLATION_FACETS: FacetGroup<CancellationRow>[] = [
  {
    title: "Dates",
    icon: <CalendarRange />,
    facets: [
      { key: "cancelled", label: "Cancel Date", all: "Any date", wide: true, date: (r) => r.cancelled_date },
      createdDateFacet(),
      createdByFacet(),
    ],
  },
...
const isEntry = (r: CancellationRow) => r.id === ENTRY_ID;

export function CancellationsTable({ cancellations, orders, buyers }: Props) {
  ...
  const facets = useFacetFilter(cancellations, CANCELLATION_FACETS);
  const matchesFacets = facets.matches;
  const filtered = useMemo(() => {
    const needle = query.trim().toLowerCase();
    return cancellations.filter((r) => {
  ...
  }, [cancellations, query, matchesFacets]);
  ...
  /* "New Cancellation" from the sidebar / command palette lands on the entry
     ...
     the Sheet. */
  useCreateIntent(() => {
    const first = document.getElementById("goc-order") as HTMLElement | null;
    first?.focus();
  });
  ...
  const columns: Column<CancellationRow>[] = [
    {
      ...
      cell: (row) =>
        isEntry(row) ? (
          <Field w="num">
            <Input id="goc-cancelno" value="(auto)" readOnly aria-label="Cancel No" />
          </Field>
        ) : (
      ...
      cell: (row) =>
        isEntry(row) ? (
          /* An `Input`, not the Sheet's `Textarea`: a table row is one line
             tall, and a remark on a cancellation is a phrase. */
          <Field w="code">
            <Input
              id="goc-remarks"
              value={remarks}
    ...
    rowActionsColumn(
      ...
      /* `w-32`, not the default `w-40`: the widest thing here is the entry
         row's "Cancel order" (~100px), and the 32px were part of the
  ...
  const withCreated = withCreatedColumns(columns, cancellations).map((c) =>
    c.header.startsWith("Created")
      ? { ...c, cell: (r: CancellationRow) => (isEntry(r) ? null : c.cell(r)) }
      : c,
  );
  ...
  const rows: CancellationRow[] = [{ id: ENTRY_ID } as CancellationRow, ...filtered];
  ...
  return (
    <div className="space-y-3">
      <FilterBar
        search={query}
        onSearch={setQuery}
        searchPlaceholder="Search Cancel No, RE No, Order No or customer…"
        activeCount={facets.activeCount}
        onReset={facets.activeCount ? facets.reset : undefined}
        panel={facets.panel}
        right={`${filtered.length} of ${cancellations.length}`}
      />
      <DataTable
        columns={withCreated}
        rows={rows}
        ...
        empty={cancellations.length ? "No cancellations match these filters." : "No cancellations yet."}
      />
    </div>
  );
}
