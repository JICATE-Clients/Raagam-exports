...
import {
  addProductionPlanOrder,
  updateProductionPlanOrder,
  deleteProductionPlanOrder,
  submitProductionPlan,
  approveProductionPlan,
  deleteProductionPlan,
} from "@/lib/planning/production-planning-actions";
import type { getProductionPlan } from "@/lib/planning/production-planning-service";
import type { ProductionPlanOrder, PpStatus } from "@/lib/planning/production-planning-types";
...
type PlanDetail = NonNullable<Awaited<ReturnType<typeof getProductionPlan>>>;
...
function rowToFields(r: ProductionPlanOrder): OrderFields {
  return {
...
function fieldsToData(
  ...
  return {
    ...
    sno: fallbackSno,
    plan_no: String(fallbackSno),
    plan_date: f.plan_date || null,
    wo_no: f.wo_no.trim() || null,
...
function OrdersTab({
  ...
  function openEdit(r: ProductionPlanOrder) {
  ...
  const columns: Column<OrderRow>[] = [
    ...
    { header: "Plan No",   cell: (r) => <span className="text-sm">{r.plan_no ?? "--"}</span> },
    { header: "WO No",     cell: (r) => <span className="text-sm">{r.wo_no ?? "--"}</span> },
          ...
          <div className="rounded-md border border-border bg-surface-muted p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
              ...
              <div>
                ...
                <Input
                  value={form.wo_no}
                  onChange={(e) => setForm((f) => ({ ...f, wo_no: e.target.value }))}
                />
              </div>
              <div>
                <Label>SC No</Label>
...
export function ProductionPlanningDetail({
  plan,
  canEdit,
  canDelete,
  canApprove,
  ...
  function handleAddOrder(data: Record<string, unknown>) {
    startTransition(async () => {
      const res = await addProductionPlanOrder(data);
  ...
  function handleUpdateOrder(orderId: string, data: Record<string, unknown>) {
    startTransition(async () => {
      const res = await updateProductionPlanOrder(orderId, plan.id, data);
  ...
  function handleDeleteOrder(orderId: string) {
    startTransition(async () => {
      const res = await deleteProductionPlanOrder(orderId, plan.id);
  ...
  function handleSubmit() {
    startTransition(async () => {
      const res = await submitProductionPlan(plan.id);
  ...
  function handleApprove() {
    startTransition(async () => {
      ...
      if (res.ok) { success("Production Plan approved."); router.refresh(); }
      else toastError(res.error);
    });
  }
  ...
  function handleDelete() {
    startTransition(async () => {
      const res = await deleteProductionPlan(plan.id);
      if (res.ok) { success("Deleted."); router.push("/planning/production-planning"); }
      else toastError(res.error);
    });
  }
