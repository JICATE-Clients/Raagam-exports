...
export function BeamTypeMasterScreen({ rows, perms }: { rows: Row[]; perms: Perms }) {
  ...
  function submit() {
    startTransition(async () => {
      ...
      const res = editId
        ? await updateBeamType(editId, payload)
        : await createBeamType(payload);
      if (res.ok) {
        success(editId ? "Beam type updated." : "Beam type added.");
        setOpen(false);
        router.refresh();
      } else {
        error(res.error);
  ...
  function remove(r: Row) {
    startTransition(async () => {
      const res = await deleteBeamType(r.id);
      if (res.ok) {
        success(res.inactive ? "Beam type marked inactive." : "Beam type deleted.");
        router.refresh();
      } else {
        error(res.error);
      }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <FilterBar
          ...
          searchPlaceholder="Search beam type..."
          activeCount={activeCount}
          onReset={() => { reset(); pg.setPage(1); }}
        >
          <div>
            <Label htmlFor="bt-filter-status">Status</Label>
            <Select
              id="bt-filter-status"
        ...
        <div className="flex flex-1 items-center justify-end gap-2">
          <DataIoToolbar entityKey="beam-types" rows={filtered} canExport={perms.canExport} />
          {perms.canCreate && (
      ...
      <div className="hidden md:block">
        <DataTable columns={columns} rows={pg.paged} getKey={(r) => r.id} empty="No beam type records yet." />
      </div>

      <div className="space-y-2.5 md:hidden">
        {pg.paged.length === 0 ? (
          <div className="rounded-lg border border-border bg-surface px-4 py-10 text-center text-sm text-muted-foreground">
            No beam type records yet.
          </div>
        ) : (
          pg.paged.map((r) => (
            <button
      ...
      <Sheet
        ...
        title={editId ? "Edit Beam Type" : "New Beam Type"}
        footer={
          <>
