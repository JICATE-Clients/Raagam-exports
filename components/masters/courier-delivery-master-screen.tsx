...
export function CourierDeliveryAddressMasterScreen({
  ...
  const [viewRow, setViewRow] = useState<CourierDeliveryAddress | null>(null);
   ...
   *
   * Identical to notify-master-screen's by design, not by copy-paste drift: the
   * two masters hold the same record. Change one, change both.
   */
  function viewSections(r: CourierDeliveryAddress): ViewSection[] {
        ...
        sections={[
          ...
          },
          {
            ...
            <DetailSection label="Communication" cols={12}>
              <Field label="Land Line" size={FIELD_SIZE.land_line} htmlFor="cda-landline">
                <Input
                  id="cda-landline"
              ...
              <Field label="E-Mail" size={FIELD_SIZE.email} htmlFor="cda-email">
                <ValidatedInput
                  format="email"
                  id="cda-email"
                  value={form.email}
              ...
              <Field label="Web site" size={FIELD_SIZE.web_site} htmlFor="cda-web">
                <ValidatedInput
                  format="website"
                  id="cda-web"
                  value={form.web_site}
          ...
          },
          {
            ...
            {/* Seven fields per row, so stacked cards (LAYOUT.md §6) with a
                ...
                Two rows per card, not three (client 2026-07-29). The four
                pickers were `lg` and the three contact channels `md` — 6+6 /
