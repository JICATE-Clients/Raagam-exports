 ...
 * + "Customers" button on the Consignee form): a searchable Short-Name/Name grid
 * with OK / Cancel (double-click to pick). No inline Add/Modify — a Customer is a
 * rich master edited on its own screen, so here we only reference existing ones.
 */
export function CustomerPicker({
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ? customers.filter((c) => [c.code, c.name].filter(Boolean).join(" ").toLowerCase().includes(q))
      : customers;
