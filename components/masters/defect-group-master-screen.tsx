...
export function DefectGroupMasterScreen({ rows, perms }: { rows: Row[]; perms: Perms }) {
  ...
  function submit() {
    startTransition(async () => {
      ...
      const res = editId
        ? await updateDefectGroup(editId, payload)
        : await createDefectGroup(payload);
      if (res.ok) {
        success(editId ? "Defect group updated." : "Defect group added.");
        setOpen(false);
        router.refresh();
      } else {
        error(res.error);
  ...
  function remove(r: Row) {
    startTransition(async () => {
      const res = await deleteDefectGroup(r.id);
      if (res.ok) {
        success(res.inactive ? "Defect group marked inactive." : "Defect group deleted.");
        router.refresh();
      } else {
        error(res.error);
      }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <FilterBar
          ...
          searchPlaceholder="Search defect group..."
          activeCount={activeCount}
          onReset={() => { reset(); pg.setPage(1); }}
        >
          <div>
            <Label htmlFor="dg-filter-status">Status</Label>
            <Select
              id="dg-filter-status"
        ...
        <div className="flex flex-1 items-center justify-end gap-2">
          <DataIoToolbar entityKey="defect-groups" rows={filtered} canExport={perms.canExport} />
          {perms.canCreate && (
      ...
      <div className="hidden md:block">
        <DataTable columns={columns} rows={pg.paged} getKey={(r) => r.id} empty="No defect group records yet." />
      </div>

      <div className="space-y-2.5 md:hidden">
        {pg.paged.length === 0 ? (
          <div className="rounded-lg border border-border bg-surface px-4 py-10 text-center text-sm text-muted-foreground">
            No defect group records yet.
          </div>
        ) : (
          pg.paged.map((r) => (
            <button
      ...
      <Sheet
        ...
        title={editId ? "Edit Defect Group" : "New Defect Group"}
        footer={
          <>
