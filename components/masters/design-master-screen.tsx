...
const BLANK = { name: "", inactive: false };
...
export function DesignMasterScreen({ rows, perms }: { rows: Row[]; perms: Perms }) {
  ...
  function openEdit(r: Row) {
    setEditId(r.id);
    setForm({ name: r.name, inactive: !r.is_active });
    setOpen(true);
  }

  function submit() {
    startTransition(async () => {
      const payload = { name: form.name.trim(), is_active: !form.inactive };
      const res = editId
        ? await updateDesign(editId, payload)
        : await createDesign(payload);
      if (res.ok) {
        success(editId ? "Design updated." : "Design added.");
        setOpen(false);
        router.refresh();
      } else {
        error(res.error);
  ...
  function remove(r: Row) {
    startTransition(async () => {
      const res = await deleteDesign(r.id);
      if (res.ok) {
        success(res.inactive ? "Design marked inactive." : "Design deleted.");
        router.refresh();
      } else {
        error(res.error);
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <div>
            <Label htmlFor="des-filter-status">Status</Label>
            <Select
              id="des-filter-status"
        ...
        <div className="flex flex-1 items-center justify-end gap-2">
          <DataIoToolbar entityKey="designs" rows={filtered} canExport={perms.canExport} />
          {perms.canCreate && (
      ...
      <div className="hidden md:block">
        <DataTable columns={columns} rows={pg.paged} getKey={(r) => r.id} empty="No design records yet." />
      </div>
      ...
      <Sheet
        ...
        title={editId ? "Edit Design" : "New Design"}
        footer={
          <>
