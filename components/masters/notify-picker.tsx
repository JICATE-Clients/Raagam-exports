...
/**
 * Select-only ⓘ picker over the `notifies` master (the Consignee "Notify" tab
 * grid): a searchable Short-Name/Name grid with OK / Cancel (double-click to
 * pick). No inline Add/Modify — a Notify party is a rich master edited on its
 * own screen. Mirrors ApplicantPicker / CustomerPicker.
 */
export function NotifyPicker({
  ...
  const selected = notifies.find((n) => n.id === value) ?? null;

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    const rows = q
      ? notifies.filter((n) => [n.code, n.name].filter(Boolean).join(" ").toLowerCase().includes(q))
      : notifies;
