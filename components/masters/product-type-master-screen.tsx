...
/**
 * Product Type master — simple flat list (Code req unique · Name · Inactive).
 * Backed by the `product_types` table. Code is required and unique; if Name is
 * left blank it defaults to Code on save (auto-copy behaviour).
 */
export function ProductTypeMasterScreen({ rows, perms }: { rows: Row[]; perms: Perms }) {
  ...
  function submit() {
    startTransition(async () => {
      ...
      // Auto-copy code to name if name is left blank
      ...
      if (res.ok) {
        success(editId ? "Product type updated." : "Product type added.");
        setOpen(false);
        router.refresh();
      } else {
        error(res.error);
  ...
  function remove(r: Row) {
    startTransition(async () => {
      const res = await deleteProductType(r.id);
      if (res.ok) {
        success(res.inactive ? "Product type marked inactive." : "Product type deleted.");
        router.refresh();
      } else {
        error(res.error);
      }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex flex-1 items-center justify-end gap-2">
          <DataIoToolbar entityKey="product-types" rows={filtered} canExport={perms.canExport} />
          {perms.canCreate && (
      ...
      <div className="hidden md:block">
        <DataTable
          ...
          rows={pg.paged}
          getKey={(r) => r.id}
          empty="No product type records yet."
        />
      </div>
      ...
      <Sheet
        ...
        title={editId ? "Edit Product Type" : "New Product Type"}
        footer={
          <>
