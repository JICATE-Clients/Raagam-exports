...
| **B · Contract & guards** | ✅ **DONE 2026-09-22.** `lib/orders/amendments/amendment-entry.ts` (the six types, the seed mirror, `unionScope`, `openAreasOf`, `entryStatusOf`, `marginDelta`); `assertOrderUnlocked` → `assertOrderWritable(orderId, area)` at all five call sites; the D6 sentence in TS + vectors; `npm run check:amendment-scope` (in `build:check`, made to FAIL first). |
| **C · Screens** | ✅ **DONE 2026-09-22.** `UnlockScope` + the `open` set in `field.tsx` / `MasterFullScreen`; the Order Entry editor unlocks by section and by header field (`delivery_date`, `excess_pct`, `money_terms`); the BOM pages ask `orderLockMessages(…, area)`; **Orders ▸ Order Amendments** (`/orders/order-amendments`, a `link` row): the register, the Raise sheet (multi-category, scope preview), the entry page (variance matrix, what changed, downstream freshness, approval timeline, Abandon); `[Amend]` on the order list; `?open=` on the four editors. |
| **D · Propagation** | ✅ **DONE 2026-09-22.** `0616_order_amendments_register.sql` (multi-category union scope, the trigger reads the FROZEN scope, V0 order snapshot + `order_amendment_changes`, `abandon_order_amendment`, `approval_apply_terminal` closes the entry on re-approval); the BOM-freshness gate in `refuseUnreadyOrders`; the amendment block on the approval run's context and the MD push wording. Verified by 0616's rolled-back behaviour probe (ran, not skipped: 2 candidate orders). |

**What the second spec (`doc/order/amedment.md`, 2026-09-22) added on top, and how it landed:** the register with Margin Delta and a derived Status; the Change Category as a MULTI-select (union scope); the V0 snapshot of the whole order + both BOMs, diffed for "what changed" instead of a logging trigger; the side-by-side variance matrix (`compareToBaseline` + the two KPI sets); the MD gate (the run's `context.amendment`, the push naming the entry and the margin move, `refuseReopenedBudget` naming the entry — the PO hard lock was already standing). NOT built, deliberately: a generic table-by-table RESTORE from the snapshot on reject (T&A rows reference locked rows; a delete-and-reinsert restore would cascade through them) — Abandon restores V0 only when nothing changed, otherwise the RE stays open for a fresh approval.
