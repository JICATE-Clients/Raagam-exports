...
function rev(): void {
  ...
  revalidatePath("/masters/associates/courier-delivery-address");
}

type ContactRow = Omit<CourierDeliveryInput["contacts"][number], "sno"> & { sno: number };

/** Drop fully-empty contact rows (no picker + all text blank) and renumber sno. */
function normalizeContacts(data: CourierDeliveryInput): ContactRow[] {
...
export async function createCourierDeliveryAddress(data: CourierDeliveryInput): Promise<Result> {
  ...
  if (rows.length) {
    const { error: cErr } = await s
      .from("courier_delivery_contacts")
      .insert(rows.map((r) => ({ ...r, courier_id: created.id })));
...
export async function updateCourierDeliveryAddress(
  id: string,
  data: CourierDeliveryInput,
): Promise<Result> {
  ...
  if (rows.length) {
    const { error: cErr } = await s
      .from("courier_delivery_contacts")
      .insert(rows.map((r) => ({ ...r, courier_id: id })));
    if (cErr) return fail(cErr.message);
  }
  rev();
  return { ok: true };
