...
export async function createCountGroup(data: CountGroupInput): Promise<CreateResult> {
  ...
  const dupName = await checkDuplicateName(s, "count_groups", name, { nameColumn: "name" });
...
export async function updateCountGroup(id: string, data: CountGroupInput): Promise<Result> {
  ...
  if (!name) return fail("Name is required.");
  const s = await createClient();
  const dupName = await checkDuplicateName(s, "count_groups", name, {
    nameColumn: "name",
    excludeId: id,
  });
  if (!dupName.ok) return fail(dupName.error);
...
export async function createConstruction(data: ConstructionInput): Promise<CreateResult> {
  ...
  if (!name) return fail("Name is required.");
  const s = await createClient();
  const dupName = await checkDuplicateName(s, "constructions", name, { nameColumn: "name" });
  if (!dupName.ok) return fail(dupName.error);
  if (!code) {
...
export async function updateConstruction(id: string, data: ConstructionInput): Promise<Result> {
  ...
  if (!name) return fail("Name is required.");
  const s = await createClient();
  const dupName = await checkDuplicateName(s, "constructions", name, {
    nameColumn: "name",
    excludeId: id,
  });
  if (!dupName.ok) return fail(dupName.error);
...
export async function createProcessSequence(data: ProcessSequenceInput): Promise<CreateResult> {
  ...
  const s = await createClient();
  const dupName = await checkDuplicateName(s, "process_sequences", name, { nameColumn: "name" });
  if (!dupName.ok) return fail(dupName.error);
  if (!code) {
...
export async function updateProcessSequence(id: string, data: ProcessSequenceInput): Promise<Result> {
  ...
  if (!name) return fail("Name is required.");
  const s = await createClient();
  const dupName = await checkDuplicateName(s, "process_sequences", name, {
    nameColumn: "name",
    excludeId: id,
  });
  if (!dupName.ok) return fail(dupName.error);
...
export async function createProcessSequenceGroup(data: ProcessSequenceGroupInput): Promise<CreateResult> {
  ...
  const s = await createClient();
  const dupName = await checkDuplicateName(s, "process_sequence_groups", name, {
    nameColumn: "name",
  });
  if (!dupName.ok) return fail(dupName.error);
  if (!code) {
