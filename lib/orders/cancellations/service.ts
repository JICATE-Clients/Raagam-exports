...
export type CancellationRow = OrderCancellation & {
  sales_orders: {
    order_number: string | null;
...
/** Past cancellations (newest first) with order + buyer context. */
export async function getCancellations(): Promise<CancellationRow[]> {
...
export async function getCancellableOrders(): Promise<OrderOption[]> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("sales_orders")
