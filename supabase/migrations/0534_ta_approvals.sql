...
--
-- BACKFILLED ONTO MAIN (2026-09-09). This table has been live on the shared
-- dev database since the unmerged `ta-approvals-engine` worktree branch
-- applied it directly — main's own 0542-0546 already `alter table
-- public.ta_approvals ...`, which only works if this table exists first. This
-- file closes that hole in main's migration ledger so a fresh environment
-- replays in the right order; content is unchanged from the worktree's
-- original 0534, and every statement is idempotent, so it is a no-op against
-- the dev database this session is pointed at.
--
...
begin
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='ta_approvals' and policyname='ta_approvals_read') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='ta_approvals' and policyname='ta_approvals_insert') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='ta_approvals' and policyname='ta_approvals_update') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='ta_approvals' and policyname='ta_approvals_delete') then
