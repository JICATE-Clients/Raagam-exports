...
-- unchanged from the worktree original (this is the PRE-0543 shape: `status`
-- still admits only pending/sent/received, no active_version/mime_type/
-- size_bytes) — main's own 0543 already alters this table forward from
-- exactly this shape, so inserting the later shape here would make 0543's own
-- `alter table ... add column if not exists` statements diverge from what
-- they were written and verified against. Policy creation is additionally
-- guarded so this file is a true no-op against a database where the table
-- and policies already exist.
--
...
begin
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='garment_order_amendment_ta_approvals' and policyname='goa_ta_approvals_read') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='garment_order_amendment_ta_approvals' and policyname='goa_ta_approvals_insert') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='garment_order_amendment_ta_approvals' and policyname='goa_ta_approvals_update') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='garment_order_amendment_ta_approvals' and policyname='goa_ta_approvals_delete') then
